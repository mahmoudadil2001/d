...
function updateVersionSelector() {
  ...
  // Check if user has access (signed in with VIP OR free trial active)
  const hasAccess = (isUserSignedIn && vipMode) || freeTrialActive;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // التحقق من تسجيل الدخول أو التجربة المجانية قبل تفعيل ميزات الترتيب العشوائي
  const isUserSignedIn = authManager &&
                        authManager.currentUser &&
                        authManager.currentUser.uid &&
                        authManager.isSignedIn();

  const hasShuffleAccess = (isUserSignedIn && vipMode) || freeTrialActive;
...
function initializeFreeTrial() {
  ...
  if (!trialStartTime && !trialUsed) {
    ...
    if (elapsedTime < 180) { // 180 seconds = 3 minutes
      ...
      freeTrialTimeLeft = Math.ceil(180 - elapsedTime);
      startFreeTrialTimer();
    } else {
      // Trial expired
      endFreeTrial();
...
function startFreeTrial() {
  freeTrialActive = true;
  freeTrialTimeLeft = 180; // 3 minutes in seconds
...
function showTrialActivationMessage() {
  ...
  message.innerHTML = `
    ...
    <div style="font-size: 16px; line-height: 1.5; margin-bottom: 20px;">
      <div style="margin: 8px 0;">✅ جميع نسخ الأسئلة متاحة</div>
      <div style="margin: 8px 0;">🔀 الترتيب العشوائي مفعل</div>
      <div style="margin: 8px 0;">⚡ مميزات VIP لمدة دقيقة كاملة</div>
    </div>
    <div style="font-size: 14px; opacity: 0.9; font-weight: 600;">
      ✨ استمتع بتجربة التعلم المتقدمة! ✨
    </div>
  `;
...
function showTrialExpiredNotification() {
  ...
  notification.innerHTML = `
    ...
    <div style="margin-bottom: 15px; font-size: 18px; font-weight: 700;">انتهت التجربة المجانية</div>
    <div style="font-size: 14px; opacity: 0.9; line-height: 1.4; margin-bottom: 18px;">
      سجل دخولك للحصول على المزيد من المميزات
    </div>
...
function updateFreeTrialBadge() {
  ...
  if (freeTrialActive && freeTrialTimeLeft > 0) {
    ...
    // إخفاء/إظهار الشارة حسب وضع الاختبار
    if (isQuizMode) {
...
function showVipSubscriptionModal() {
    ...
    ">
      ...
      <div style="padding: 30px;">
        ...
        <div style="margin-bottom: 30px;">
          ...
          <div class="vip-features" style="display: none; grid-gap: 15px; display: grid;">
            
            <!-- بطاقة المظهر الفاتح -->
            <div class="vip-feature" style="background: rgba(255, 255, 255, 0.2); padding: 15px; border-radius: 12px; border-left: 4px solid #fff; cursor: pointer;">
  ...
  setTimeout(() => {
    ...
    // Function to toggle the features section
    function toggleFeatures() {
      if(vipFeatures.style.display === 'none' || vipFeatures.style.display === ''){
        vipFeatures.style.display = 'grid';
        expandIndicator.textContent = '▲';
      } else {
        vipFeatures.style.display = 'none';
        expandIndicator.textContent = '▼';
      }
    }

    // Initial automatic popup for 2 seconds
    vipFeatures.style.display = 'grid';
    expandIndicator.textContent = '▲';
    setTimeout(() => {
      vipFeatures.style.display = 'none';
    ...
    // Toggle individual feature descriptions
...
function applyTheme(theme) {
  ...
  if (theme === 'light') {
    ...
    questionElements.forEach(element => {
      ...
      element.style.textShadow = '0 1px 5px rgba(0, 0, 0, 0.7) !important';
    });
...
function updateShuffleControls() {
  ...
  // Check if user has access (VIP OR free trial)
  const hasShuffleAccess = (isUserSignedIn && vipMode) || freeTrialActive;

  if (hasShuffleAccess) {
    // المستخدم لديه وصول (VIP أو تجربة مجانية) - تفعيل الأزرار وإخفاء التنبيهات
    shuffleToggle.disabled = false;
    shuffleAnswersToggle.disabled = false;
    ...
    if (!isUserSignedIn && !freeTrialActive) {
      ...
      console.log('Shuffle controls disabled for non-signed-in user without free trial');
    } else if (isUserSignedIn && !vipMode) {
      // User is signed in but VIP is off and no free trial
      if (shuffleLoginHint) {
      ...
      console.log('Shuffle controls disabled - VIP mode is off and no free trial');
    }
  }
}
...
// دالة إظهار نافذة اتصل بنا مع بيانات SocialOrbit
  ...
  modal.innerHTML = `
    ...
    ">
      <div style="
        ...
        border-bottom: 2px solid rgba(255, 255, 255, 0.2);
      ">
        <h2 style="margin: 0; font-family: 'Tajawal', sans-serif; font-size: 24px; font-weight: 700;">📞 تواصل مع المطور</h2>
      ...
      <div style="padding: 25px; color: white; font-family: 'Tajawal', sans-serif; line-height: 1.6;">
        ...
        ">
          <div style="
            ...
            height: 80px;
            border-radius: 50%;
            background-image: url('./pics/picture0.jpg');
            ...
            box-shadow: 0 4px 15px rgba(0, 0, 0, 0.2);
          "></div>
          <h3 style="margin: 0 0 10px 0; font-size: 22px; font-weight: 700;">Mahmoud Adil</h3>
          <p style="margin: 0; font-size: 14px; opacity: 0.9;">طالب طب أسنان مرحلة رابعة في جامعة الإسراء</p>
        </div>
        ...
        <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 15px; margin: 25px 0;">
          <div style="
            ...
            transition: all 0.3s ease;
            border: 2px solid rgba(255, 255, 255, 0.2);
          " onclick="window.open('mailto:mahmod.adil2001@gmail.com', '_blank')">
            ...
            <h4 style="margin: 0 0 5px 0; color: #ffffff; font-size: 16px;">البريد الإلكتروني</h4>
            <p style="margin: 0; font-size: 12px; opacity: 0.8;">mahmod.adil2001@gmail.com</p>
          </div>
          ...
          <div style="
            ...
            transition: all 0.3s ease;
            border: 2px solid rgba(255, 255, 255, 0.2);
          " onclick="window.open('https://t.me/io_620', '_blank', 'noopener,noreferrer')">
            <div style="margin-bottom: 8px; display: flex; justify-content: center;">
              <svg width="32" height="32" viewBox="0 0 24 24" fill="none">
                <path d="M12 2C6.48 2 2 6.48 2 12C2 17.52 6.48 22 12 22C17.52 22 22 17.52 22 12C22 6.48 17.52 2 12 2ZM15.64 8.54L14.62 13.44C14.54 13.82 14.32 13.92 14.02 13.76L12.42 12.54L11.64 13.3C11.56 13.38 11.5 13.44 11.36 13.44L11.46 11.82L14.4 9.16C14.52 9.06 14.36 9 14.2 9.08L10.54 11.38L8.96 10.92C8.6 10.8 8.6 10.56 9.04 10.38L15.14 8.06C15.46 7.94 15.74 8.14 15.64 8.54Z" fill="#0088CC"/>
              </svg>
            </div>
            <h4 style="margin: 0 0 5px 0; color: #ffffff; font-size: 16px;">تليجرام</h4>
            <p style="margin: 0; font-size: 12px; opacity: 0.8;">@io_620</p>
          </div>
          ...
          <div style="
            ...
            transition: all 0.3s ease;
            border: 2px solid rgba(255, 255, 255, 0.2);
          " onclick="window.open('https://instagram.com/io_622', '_blank', 'noopener,noreferrer')">
            <div style="margin-bottom: 8px; display: flex; justify-content: center;">
              <svg width="32" height="32" viewBox="0 0 24 24" fill="none">
                <path d="M12 2.163C15.204 2.163 15.584 2.175 16.85 2.233C20.102 2.381 21.621 3.924 21.769 7.152C21.827 8.417 21.838 8.797 21.838 12.001C21.838 15.206 21.826 15.585 21.769 16.85C21.62 20.075 20.105 21.621 16.85 21.769C15.584 21.827 15.206 21.839 12 21.839C8.796 21.839 8.416 21.827 7.151 21.769C3.891 21.62 2.38 20.07 2.232 16.849C2.174 15.584 2.162 15.205 2.162 12C2.162 8.796 2.175 8.417 2.232 7.151C2.381 3.924 3.896 2.38 7.151 2.232C8.417 2.175 8.796 2.163 12 2.163ZM12 7C9.239 7 7 9.239 7 12C7 14.761 9.239 17 12 17C14.761 17 17 14.761 17 12C17 9.239 14.761 7 12 7ZM18.406 6.594C18.406 5.853 17.8 5.247 17.059 5.247C16.318 5.247 15.712 5.853 15.712 6.594C15.712 7.335 16.318 7.941 17.059 7.941C17.8 7.941 18.406 7.335 18.406 6.594C18.406 5.853 17.8 5.247 17.059 5.247C16.318 5.247 15.712 5.853 15.712 6.594C15.712 7.335 16.318 7.941 17.059 7.941C17.8 7.941 18.406 7.335 18.406 6.594C18.406 5.853 17.8 5.247 17.059 5.247C16.318 5.247 15.712 5.853 15.712 6.594C15.712 7.335 16.318 7.941 17.059 7.941C17.8 7.941 18.406 7.335 18.406 6.594ZM12 9C13.657 9 15 10.343 15 12C15 13.657 13.657 15 12 15C10.343 15 9 13.657 9 12C9 10.343 10.343 9 12 9Z" fill="url(#gradient)"/>
                <defs>
            ...
            <h4 style="margin: 0 0 5px 0; color: #ffffff; font-size: 16px;">إنستجرام</h4>
            <p style="margin: 0; font-size: 12px; opacity: 0.8;">@io_622</p>
          </div>
          ...
          <div style="
            ...
            transition: all 0.3s ease;
            border: 2px solid rgba(255, 255, 255, 0.2);
          " onclick="window.open('https://youtube.com/@mahmoud34012', '_blank', 'noopener,noreferrer')">
            <div style="margin-bottom: 8px; display: flex; justify-content: center;">
              <svg width="32" height="32" viewBox="0 0 24 24" fill="none">
                <path d="M23.498 6.186C23.245 5.264 22.551 4.541 21.667 4.281C19.78 3.75 12 3.75 12 3.75C12 3.75 4.22 3.75 2.333 4.281C1.449 4.541 0.755 5.264 0.502 6.186C0 8.131 0 12.25 0 12.25C0 12.25 0 16.369 0.502 18.314C0.755 19.236 1.449 19.959 2.333 20.219C4.22 20.75 12 20.75 12 20.75C12 20.75 19.78 20.75 21.667 20.219C22.551 19.959 23.245 19.236 23.498 18.314C24 16.369 24 12.25 24 12.25C24 12.25 24 8.131 23.498 6.186ZM9.75 15.583V8.917L15.75 12.25L9.75 15.583Z" fill="#FF0000"/>
              </svg>
            </div>
            <h4 style="margin: 0 0 5px 0; color: #ffffff; font-size: 16px;">يوتيوب</h4>
            <p style="margin: 0; font-size: 12px; opacity: 0.8;">mahmoud34012</p>
          </div>
        </div>
