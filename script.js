...
// إضافة وظيفة توسيع/إخفاء الخيارات للوضع الترفيهي
...
function updateVersionSelector() {
  ...
  // Check login status first, then VIP mode
  if (!isUserSignedIn) {
    // User is NOT logged in - show login message regardless of VIP status
  ...
  // User IS logged in - check VIP mode
  if (!vipMode) {
    // User is logged in but VIP is OFF - show VIP message
    versionSelect.style.display = "none";
    versionLoginMessage.style.display = "block";
    versionLoginMessage.innerHTML = `
    ...
    console.log('User signed in but VIP OFF - showing VIP message');
    return;
  }

  // User is logged in AND VIP is ON - show all versions
    ...
    ">
      ...
      ">
        ...
        <p style="margin: 10px 0 0 0; opacity: 0.8; font-size: 16px;">
          احصل على جميع المميزات الحصرية
        </p>
      ...
      <div style="padding: 30px;">
        ...
        <div style="margin-bottom: 30px;">
          <h3 style="margin: 0 0 20px 0; font-size: 22px; font-weight: 700; color: #212529; text-align: center;">
            🌟 مميزات اشتراك VIP
          </h3>
...
// دالة لتحديث أزرار الترتيب العشوائي حسب حالة تسجيل الدخول ووضع VIP
function updateShuffleControls() {
  ...
  // Check both login status and VIP mode
  if (isUserSignedIn && vipMode) {
    // المستخدم مسجل و VIP مفعل - تفعيل الأزرار وإخفاء التنبيهات
    ...
    // المستخدم غير مسجل أو VIP مطفأ - تعطيل الأزرار وإظهار التنبيهات
    ...
    if (!isUserSignedIn) {
      ...
      // User is signed in but VIP is off
      if (shuffleLoginHint) {
      ...
      console.log('Shuffle controls disabled - VIP mode is off');
    }
  }
}
